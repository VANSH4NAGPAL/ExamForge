...
import { Timer, FileText, Folder, Flag } from 'lucide-react';
...
export default function TestPage() {
  ...
  const [flagged, setFlagged] = useState(new Set()); // NEW: flagged question indices
  ...
  const [showSubmitModal, setShowSubmitModal] = useState(false); // NEW: submit confirmation
  ...
  }, [answers, timeLeft, examId, mode, submitted, qIndex, questions, selectedSection, flagged]);
  ...
  const handleSubmit = async () => {
    ...
    const computedResults = questions.map(q => {
      ...
      return { ...q, selectedOriginal, correctLetters, isCorrect, shuffledOptions: q.shuffledOptions };
    });
  ...
  // Called when user clicks "Submit" in the navigation or via the submit modal trigger
  const requestSubmit = () => {
                ...
                <div className="relative">
                  ...
                  {timerDropdownOpen && (
                    <div className="absolute z-10 w-full mt-1 bg-[#1A1A1A] border border-[#2A2A2A] shadow-xl">
                      <button onClick={() => { setFullTimerOption('1min'); setTimerDropdownOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-[#222] transition-colors text-fg text-sm border-b border-[#2A2A2A]/50">1 min per question</button>
                      <button onClick={() => { setFullTimerOption('unlimited'); setTimerDropdownOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-[#222] transition-colors text-fg text-sm">Unlimited time</button>
                    </div>
                  )}
                </div>
                <button onClick={() => startTest('full')} className="btn-primary">Start Full Bank →</button>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="relative mb-4">
                ...
                >
                  ...
                  <svg className={`w-4 h-4 text-muted-fg transition-transform flex-shrink-0 ${sectionDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                </button>
                {sectionDropdownOpen && (
                  <div className="absolute z-10 w-full mt-1 bg-[#1A1A1A] border border-[#2A2A2A] shadow-xl max-h-60 overflow-y-auto">
                    {sectionList.map((s, i) => {
                      ...
                      return (
                        <button key={i} onClick={() => { setSelectedSection(s); setSectionDropdownOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-[#222] transition-colors flex items-center justify-between border-b border-[#2A2A2A]/50 last:border-0">
              ...
              <button disabled={!selectedSection} onClick={() => selectedSection && startTest('section', selectedSection)} className="btn-primary disabled:opacity-30 w-full">
                Start Section Test →
              </button>
            </div>
          </div>
              ...
              {results.filter(r => !r.isCorrect).map((r, i) => (
                <div key={i}>
                  ...
                  {(r.shuffledOptions || []).map((opt, j) => {
                    ...
                    const isSelected = Array.isArray(r.selectedOriginal) ? r.selectedOriginal.includes(opt.letter) : r.selectedOriginal === opt.letter;
                    return (
                      <div key={j} className={`option-tile answered mb-2 ${isC ? 'correct' : isSelected ? 'wrong' : 'dimmed'}`}>
                        <span className="option-letter">{optionLetters[j]}</span>
  ...
  const flaggedAnsweredCount = [...flagged].filter(i => {
    const fq = questions[i];
    const v = fq && answers[fq.id];
    return v !== null && v !== undefined && (Array.isArray(v) ? v.length > 0 : true);
  }).length;
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col lg:flex-row">
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="card max-w-sm w-full">
            ...
            <p className="text-muted-fg text-sm mb-6">
              <strong className="text-fg">{answeredCount}</strong> of <strong className="text-fg">{questions.length}</strong> answered — {timeLeft === Infinity ? "Unlimited time" : <><strong className="text-fg">{formatTime(timeLeft)}</strong> remaining</>}.
            </p>
            <div className="flex flex-col gap-2">
              <button onClick={() => handleLeaveConfirm(false)} className="btn-primary w-full justify-center">Save & Leave</button>
              <button onClick={() => handleLeaveConfirm(true)} className="btn-ghost w-full justify-center text-[#EF4444] border-border hover:border-[#EF4444]">Discard & Leave</button>
              <button onClick={() => setShowLeaveModal(false)} className="btn-ghost w-full justify-center mt-1">Continue Test</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showSubmitModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="card max-w-sm w-full">
            <h2 className="text-fg font-bold text-lg mb-2">Submit Test?</h2>
            <p className="text-muted-fg text-sm mb-4 leading-relaxed">
              <strong className="text-fg">{answeredCount}</strong> of <strong className="text-fg">{questions.length}</strong> questions answered.
              {answeredCount < questions.length && <span className="text-[#EF4444]"> {questions.length - answeredCount} unanswered!</span>}
            </p>
            {flagged.size > 0 && (
              <div className="bg-[#2A2000] border border-[#FFD500]/30 p-3 mb-4 rounded">
                <div className="flex items-center gap-2 text-[#FFD500] text-sm font-bold mb-1">
                  <Flag size={14} /> {flagged.size} Flagged Question{flagged.size > 1 ? 's' : ''}
                </div>
                <p className="text-muted-fg text-xs">You have flagged questions for review. Make sure you&apos;ve reviewed them before submitting.</p>
              </div>
            )}
            <div className="flex flex-col gap-2">
              <button onClick={handleSubmit} className="btn-primary w-full justify-center">Submit & See Results →</button>
              <button onClick={() => setShowSubmitModal(false)} className="btn-ghost w-full justify-center">Go Back & Review</button>
            </div>
          </div>
        </div>
      )}
      ...
      <aside className="sidebar">
        ...
        <div className="mx-3 my-2 p-3 bg-[#141414] border border-border">
          ...
          <div className="progress-bar-bg mt-2">
            <div className="progress-bar-fill" style={{ width: `${(answeredCount / questions.length) * 100}%` }}></div>
          </div>
          <div className="text-[10px] text-muted-fg mt-1">{answeredCount} / {questions.length} answered</div>
        </div>

        <div className="px-3 mb-2 flex flex-col gap-2">
          <button onClick={handleLeave} className="btn-ghost w-full justify-center text-xs py-2">← Leave Session</button>
          <button onClick={requestSubmit} className="btn-primary w-full justify-center text-xs py-2">Submit Test →</button>
        </div>

        <div className="sidebar-section-label">
          Questions
          {flagged.size > 0 && <span className="ml-2 text-[#FFD500] text-[10px]">⚑ {flagged.size} flagged</span>}
        </div>
        <div className="px-3 grid grid-cols-5 gap-1 pb-4">
          {questions.map((q, i) => {
            const ans = answers[q.id];
            const hasAnswer = ans !== null && ans !== undefined && (Array.isArray(ans) ? ans.length > 0 : true);
            const isFlag = flagged.has(i);
            return (
              <button
                key={i}
                onClick={() => setQIndex(i)}
                className={`aspect-square text-xs font-bold transition-colors relative ${
                  i === qIndex ? 'bg-[#FFD500] text-black' :
                  hasAnswer ? 'bg-[#1A3A1A] text-[#22C55E]' :
                  'bg-[#1A1A1A] text-muted-fg hover:bg-[#222] hover:text-fg'
                }`}
              >
                {i + 1}
                {isFlag && (
                  <span className="absolute -top-1 -right-1 text-[#FFD500] text-[8px] leading-none">⚑</span>
                )}
              </button>
            );
          })}
        ...
        <div className="max-w-3xl w-full">
          ...
          <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
            <div className="flex items-center gap-2 text-xs text-muted-fg flex-wrap">
              ...
              <span className="text-muted-fg">Q {qIndex + 1} / {questions.length}</span>
            </div>
            <div className="flex gap-2 flex-wrap">
              <button onClick={handleLeave} className="btn-ghost text-xs py-2 px-3">← Leave</button>
              <button
                onClick={() => toggleFlag(qIndex)}
                className={`btn-ghost text-xs py-2 px-3 flex items-center gap-1 ${isFlagged ? 'text-[#FFD500] border-[#FFD500]/50' : ''}`}
              >
                <Flag size={12} />
                {isFlagged ? 'Flagged' : 'Flag'}
              </button>
              <button onClick={requestSubmit} className="btn-primary text-xs">Submit Test →</button>
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="flex gap-2 items-center flex-wrap mb-4">
              ...
              {isFlagged && (
                <span className="badge" style={{ background: '#2A2000', color: '#FFD500', border: '1px solid rgba(255,213,0,0.3)' }}>
                  ⚑ Flagged for Review
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-3 mt-6 flex-wrap">
            ...
            {isLastQuestion && allAnswered ? (
              <button onClick={requestSubmit} className="btn-primary animate-pulse">
                Submit Test →
              </button>
            ) : (
              <button onClick={() => setQIndex(q => Math.min(questions.length - 1, q + 1))} className="btn-primary" disabled={isLastQuestion}>
                Next →
              </button>
            )}
          </div>
